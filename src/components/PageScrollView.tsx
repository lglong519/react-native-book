...
builtinStyleArgs={
	...
	rotateChangeMode: {
		rS: 45, skewS: 45, oS: 0.5, sS: 0.5, bg: "#fff"
	},
};
...
builtinStyles={
	sizeChangeMode: (index, data, { size, opacity }) => {
		...
		const { HorV, builtinWH } = this.props;
		const isH = HorV === "h";
	...
	rotateChangeMode: (index, data, { rotate, opacity, size }) => {
		...
		const { rS, sS } = this.builtinStyleArgs.rotateChangeMode;
		const { viewWidth, viewHeight } = this.state;
		const { HorV, builtinWH } = this.props;
		const image = ImageBackground || Image;
		const isH = HorV === "h";
		let customW = null; let
			customH = null;
		if (builtinWH) { customW = builtinWH.width; customH = builtinWH.height; }
		const widimage = customW || (isH ? w * 0.5 : w * 0.7); const wid1 = isH ? widimage : viewWidth; const hei1 = isH ? viewHeight : customH || (widimage / 16 * 9); const heimage = customH || (widimage / 16 * 9); let hei = hei1; let
			wid = wid1;
		...
		const SH = Math.cos(Math.PI * (rS / 180)) * hei1 * sS; const
		...
		return (
			<View
				...
				<image source={data} style={{ width: widimage * size, height: heimage * size }}>
					<View style={{
						position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "#000", opacity: 1 - opacity
					}}/>
				</image>
			</View>
		);
...
render() {
	const { style, builtinStyle } = this.props;
	const { builtinStyles, builtinStyleArgs } = this;
	return (
...
// 渲染下面的指示器
renderPointer=() => {
	const { pointerColor: pc, renderPointView } = this.props;
	const viewArr = [];
	const currentPage = this.getintCP();

	const datas = this.props.imageArr.length ? this.props.imageArr : this.props.datas;
	...
	return <View style={[{
		position: "absolute", bottom: this.state.viewHeight / 10, flexDirection: "row", width: this.state.viewWidth, justifyContent: "center"
...
renderScrollView=() => {
	let {
		imageArr, datas, ifInfinite, dealWithClickImage: dealClick, sizeSmall, opacitySmall, rotateDeg, skewDeg, imageStyle, resizeMode, HorV, builtinStyle
	} = this.props;
	const {
		viewWidth, width, viewHeight, height, pageNum, currentPage
	} = this.state;
	const { builtinStyles, builtinStyleArgs } = this;
	this.dealWithIntCP();
	// datas数据
	...
	let skewS = skewDeg;
	if (builtinStyle) {
		const args = builtinStyleArgs[builtinStyle];
		({
			sS, oS, rS, skewS
		} = args);
	}
	...
	for (let i = 0; i < datas.length; i++) {
		...
		if (this.props.imageArr.length) {
			const style = imageStyle || { width: viewWidth, height: viewHeight };
			if (builtinStyle) {
				arr.push(
				...
				arr.push(
					<TouchableOpacity activeOpacity={dealClick ? 0.5 : 1} onPress={() => dealClick && dealClick(i % this.pNum)} key={i} style={style} onLayout={this.imageLayout}>
						<Image source={datas[i]} style={[style, { resizeMode }]}/>
					</TouchableOpacity>
				);
			}
		} else {
...
onScrollEndDrag=(e) => {
	...
	const speed = this.props.goToNextPageSpeed;
	const contentOffset = this.isH ? e.nativeEvent.contentOffset.x : e.nativeEvent.contentOffset.y;
	const scrollArr = this.isH ? this.state.scrollXArr : this.state.scrollYArr;
	const speed1 = scrollArr[scrollArr.length - 1] - scrollArr[scrollArr.length - 2];
	const speed2 = scrollArr[scrollArr.length - 2] - scrollArr[scrollArr.length - 3];
	const speed3 = scrollArr[scrollArr.length - 3] - scrollArr[scrollArr.length - 4];
	const speed4 = scrollArr[scrollArr.length - 1] - scrollArr[0];
	let currentPage;
...
scrollToPage=(currentPage, animated = true) => {
	const pageNum = this.props.ifInfinite ? this.pNum * 3 : this.pNum;
	if (currentPage > pageNum - 1) { currentPage = pageNum - 1; }
	if (currentPage < 0) { currentPage = 0; }
	try {
		this.isH ? this.scrollView.scrollTo({ x: currentPage * this.distance, animated }) : this.scrollView.scrollTo({ y: currentPage * this.distance, animated });
	} catch (e) {
		//
	}
};
...
// 设置轮播的定时器
setInfiniteInterval=() => {
	const interval = this.props.infiniteInterval < 1000 ? 1000 : this.props.infiniteInterval;
